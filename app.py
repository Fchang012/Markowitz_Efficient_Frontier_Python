...
def parse_brokerage_csv(uploaded_file):
    try:
        ...
        for col in df.columns:
            ...
            new_cols.append(COLUMN_MAP.get(clean_col, clean_col))
        df.columns = new_cols
        ...
        cash_tickers = {'CASH', 'FCASH', 'SPAXX', 'VMFXX', 'VMMXX', 'FDRXX', 'FZFXX', 'SPRXX'}
        df = df[~df['ticker'].isin(cash_tickers)]
        df = df[~df['ticker'].str.endswith('**', na=False)]
        
        footer_indicators = {'TOTAL', 'PENDING', 'PENDING ACTIVITY', 'NAN', 'N/A'}
        df = df[~df['ticker'].isin(footer_indicators)]
...
# --- TAB 3: TOTAL PORTFOLIO CALCULATOR ---
...
if active_tool == "💼 Total Portfolio Calculator" and 'run_pc' in locals() and run_pc and st.session_state.get("pc_parsed_holdings") is not None:
    ...
    with col2:
        ...
        if max_concentration > 0.25:
            ...
            st.warning(f"⚠️ **Asset Concentration Risk**: Your top holding ({top_holding}) makes up {max_concentration:.1%} of your portfolio.")