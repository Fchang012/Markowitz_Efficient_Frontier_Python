...
_COLUMN_MAP = {
    "symbol": "ticker", "ticker": "ticker", "ticker symbol": "ticker",
    "shares": "shares", "quantity": "shares", "qty": "shares", "qty #": "shares", "units": "shares",
    "description": "name", "investment name": "name", "investment": "name", "fund": "name", "fund name": "name",
    "account number": "account", "account name": "account", "account name/number": "account", "account": "account",
    "institution": "institution", "brokerage": "institution",
}
_CASH_TICKERS = {'CASH', 'FCASH', 'SPAXX', 'VMFXX', 'VMMXX', 'FDRXX', 'FZFXX', 'SPRXX'}
_FOOTER_INDICATORS = {'TOTAL', 'PENDING', 'PENDING ACTIVITY', 'NAN', 'N/A'}
...
def parse_brokerage_csv(uploaded_file):
    try:
        ...
        if uploaded_file.name.endswith('.xlsx'):
            ...
            header_idx = -1
            keywords = {'symbol', 'ticker', 'ticker symbol', 'shares', 'quantity', 'qty', 'units', 'description', 'fund name', 'investment name', 'account', 'institution', 'brokerage'}
            for i, line in enumerate(lines):
                row_vals = set([v.strip().strip('"').lower() for v in line.split(',')])
                if len(row_vals.intersection(keywords)) >= 2:
                    header_idx = i
                    break
                    
            if header_idx == -1:
                return None, None, "Could not find recognizable column headers."
            
            csv_content = "\n".join(lines[header_idx:])
            df = pd.read_csv(io.StringIO(csv_content))
            
        original_cols = set([str(c).strip().lower() for c in df.columns])
        
        detected_source = "Unknown (Generic)"
        if {"quantity", "description", "cost basis per share"}.issubset(original_cols):
            detected_source = "Fidelity"
        elif {"last price $", "value $", "price paid $"}.intersection(original_cols):
            detected_source = "E*Trade"
        elif {"ticker symbol", "fund name"}.issubset(original_cols):
            detected_source = "T. Rowe Price"
        ...
        for col in df.columns:
            ...
            for char in ['$', '%', '#']:
                clean_col = clean_col.replace(char, '').strip()
            new_cols.append(_COLUMN_MAP.get(clean_col, clean_col))
        df.columns = new_cols
        
        if "ticker" not in df.columns or "shares" not in df.columns:
            return None, None, f"Missing required columns. Found: {', '.join(df.columns)}"
        ...
        df = df[~df['ticker'].isin(_CASH_TICKERS)]
        df = df[~df['ticker'].str.endswith('**', na=False)]
        
        df = df[~df['ticker'].isin(_FOOTER_INDICATORS)]
        
        return df, detected_source, None
    except Exception as e:
        return None, None, f"Error parsing file: {e}"
...
# --- TOOL 1: TOTAL PORTFOLIO CALCULATOR ---
if "pc_parsed_holdings" not in st.session_state:
    st.session_state["pc_parsed_holdings"] = None

if active_tool == "💼 Total Portfolio Calculator":
    st.header("💼 Total Portfolio Calculator")
    st.write("Consolidate your holdings across multiple brokerages to see your true total portfolio allocation.")
    
    st.sidebar.header("1. Upload Holdings")
    
    uploaded_files = st.sidebar.file_uploader(
        "Upload CSV files from your brokerages",
        type=["csv", "xlsx"],
        accept_multiple_files=True
    )
    
    generic_template = "ticker,shares,institution\nVFIAX,150.5,Vanguard\nVTSAX,200,Vanguard\nAAPL,50,Fidelity"
    st.sidebar.download_button(
        label="📋 Download Generic Template (for Vanguard, etc.)",
        data=generic_template,
        file_name="holdings_template.csv",
        mime="text/csv"
    )
    st.sidebar.caption(
        "Vanguard does not export positions as CSV. "
        "Use this template to manually enter your Vanguard holdings."
    )
    
    all_holdings = []
    if uploaded_files:
        for file in uploaded_files:
            df, source, err = parse_brokerage_csv(file)
            if err:
                st.sidebar.error(f"{file.name} — Error: {err}")
            elif df is not None and not df.empty:
                df['source_file'] = file.name
                if 'institution' not in df.columns:
                    df['institution'] = source
                all_holdings.append(df)
                st.sidebar.success(f"{file.name} — Loaded {len(df)} holdings (detected: {source})")
        
        if all_holdings:
            combined_df = pd.concat(all_holdings, ignore_index=True)
            st.session_state["pc_parsed_holdings"] = combined_df
        else:
            st.session_state["pc_parsed_holdings"] = None
    else:
        st.session_state["pc_parsed_holdings"] = None
    ...
    if st.session_state["pc_parsed_holdings"] is not None:
        df = st.session_state["pc_parsed_holdings"]
        unique_tickers = df['ticker'].unique()
        
        st.sidebar.header("2. Price Overrides")
        ...
        if failed_tickers:
            st.sidebar.warning(f"⚠️ Could not auto-fetch prices for {len(failed_tickers)} ticker(s). Enter them manually:")
            for ticker in sorted(failed_tickers):
                manual_price = st.sidebar.number_input(
                    f"Price for {ticker}",
                    min_value=0.01,
                    value=1.0,
                    key=f"pc_manual_{ticker}"
                )
                prices[ticker] = manual_price
        
        st.sidebar.markdown("---")
        run_pc = st.sidebar.button("📊 Calculate Portfolio", key="pc_run")
        
        # Store prices so main area can use them
        if run_pc:
            st.session_state["pc_prices"] = prices



if active_tool == "💼 Total Portfolio Calculator" and 'run_pc' in locals() and run_pc and st.session_state.get("pc_parsed_holdings") is not None:
    df = st.session_state["pc_parsed_holdings"].copy()
    prices = st.session_state.get("pc_prices", {})
    ...
    if total_portfolio_value > 0:
        df['% of Portfolio'] = df['Market Value'] / total_portfolio_value
    else:
        df['% of Portfolio'] = 0.0
    ...
    m2.metric("📊 Total Unique Holdings", f"{len(df['ticker'].unique())}")
    m3.metric("🏦 Institutions", f"{len(df['source_file'].unique())}")
    
    # Display Holdings Table
    st.subheader("Holdings Detail")
    
    # Select and rename columns for display
    display_cols = ['ticker', 'shares', 'Price', 'Market Value', '% of Portfolio', 'source_file', 'institution']
    if 'name' in df.columns:
        display_cols.insert(1, 'name')
        
    display_df = df[display_cols].copy()
    display_df = display_df.sort_values(by='Market Value', ascending=False).reset_index(drop=True)
    ...
    # Download button for consolidated report
    csv_buffer = io.StringIO()
    csv_buffer.write(f'Total Market Value,"${total_portfolio_value:,.2f}"\n')
    csv_buffer.write("\n\n\n")  # Rows 2, 3, 4 blank; df header lands on row 5
    ...
    with col1:
        # Aggregate by ticker for pie chart
        ticker_agg = df.groupby('ticker')['Market Value'].sum().reset_index()
        ticker_agg = ticker_agg.sort_values(by='Market Value', ascending=False)
        
        # Group top 10 and "Other"
        top_n = 10
        if len(ticker_agg) > top_n:
            top_tickers = ticker_agg.iloc[:top_n]
            other_value = ticker_agg.iloc[top_n:]['Market Value'].sum()
            other_row = pd.DataFrame({'ticker': ['Other'], 'Market Value': [other_value]})
            plot_df = pd.concat([top_tickers, other_row])
        else:
            plot_df = ticker_agg
            
        fig1, ax1 = plt.subplots(figsize=(8, 8))
        ax1.pie(plot_df['Market Value'], labels=plot_df['ticker'], autopct='%1.1f%%', startangle=90, counterclock=False)
        ax1.axis('equal')
        st.pyplot(fig1)
        
    with col2:
        st.write("**Institution Breakdown**")
        inst_agg = df.groupby(['institution', 'source_file'])['Market Value'].sum().reset_index()
        inst_agg = inst_agg.sort_values(by='Market Value', ascending=False)
        if total_portfolio_value > 0:
            inst_agg['% of Portfolio'] = inst_agg['Market Value'] / total_portfolio_value
        else:
            inst_agg['% of Portfolio'] = 0.0
            
        st.dataframe(inst_agg.style.format({
            'Market Value': "${:,.2f}",
            '% of Portfolio': "{:.2%}"
        }))
        
        # Concentration Warning
        max_concentration = df.groupby('ticker')['Market Value'].sum().max() / total_portfolio_value if total_portfolio_value > 0 else 0
        if max_concentration > 0.25:
            top_holding = ticker_agg.iloc[0]['ticker']
            st.warning(f"⚠️ **Asset Concentration Risk**: Your top holding ({top_holding}) makes up {max_concentration:.1%} of your portfolio.")# --- TOOL 2: TAX-LOSS HARVESTING SIMULATOR ---
...
if active_tool == "🧾 Tax-Loss Harvesting Simulator" and 'run_tlh' in locals() and run_tlh and len(holdings) > 0:
    ...
    if len(all_holdings_with_prices) > 0:
        ...
        keys = ["st_gains", "st_losses", "net_st", "lt_gains", "lt_losses", "net_lt",
                "overall_net", "tax_on_st", "tax_on_lt", "total_tax",
                "ordinary_income_deduction", "ordinary_income_tax_benefit", "carryforward"]
        
        waterfall_data["Sell Gains Only"] = [f"${scenario_a[k]:,.2f}" for k in keys]
        waterfall_data["Sell All (Harvest Losses)"] = [f"${scenario_b[k]:,.2f}" for k in keys]
...
if active_tool == "📈 Portfolio Optimizer":
    ...
    po_csv = st.sidebar.file_uploader("Upload Tickers CSV", type=["csv"], key="po_csv_upload")
    po_template = "ticker,weight\nVTI,60\nVXUS,25\nBND,15"
    st.sidebar.download_button(
        "📋 Download Optimizer Template",
        data=po_template,
        file_name="optimizer_template.csv",
        mime="text/csv"
    )
    ...
    if po_csv is not None:
        csv_df = pd.read_csv(po_csv)
        csv_df.columns = [c.strip().lower() for c in csv_df.columns]
        tickers = [t.strip().upper() for t in csv_df['ticker'].dropna().tolist()]
        csv_weights = dict(zip(tickers, pd.to_numeric(csv_df['weight'], errors='coerce').tolist()))
    else:
    ...
    if tickers:
        ...
        for ticker in tickers:
            raw_val = csv_weights.get(ticker, default_weight) if csv_weights else default_weight
            default_val = float(raw_val) if raw_val is not None and not math.isnan(float(raw_val)) else default_weight
